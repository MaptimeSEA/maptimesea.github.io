...
# Intro
[Earth Engine](https://earthengine.google.com/) is a powerful platform developed by [Google](https://earth) .

We are going to be using the [Code Editor](https://code.earthengine.google.com/) which presents an easier pathway to learning the platform. The code editor utilizes the JavaScript API to run scripts and functions to interact with Earth Engine-hosted data.
